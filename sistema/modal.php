...
if ($_POST['action'] == 'searchCliente') {
  if (!empty($_POST['cliente'])) {
    ...
    $query = mysqli_query($conexion, "SELECT * FROM cliente WHERE documento LIKE '$documento'");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
if ($_POST['action'] == 'addCliente') {
  ...
  $query_insert = mysqli_query($conexion, "INSERT INTO cliente(documento, nombre, telefono, direccion, usuario_id) VALUES ('$documento','$nomnre','$telefono','$direccion','$usuario_id')");
  if ($query_insert) {
...
if ($_POST['action'] == 'addProductoDetalle') {
  if (empty($_POST['producto']) || empty($_POST['cantidad'])){
    ...
    $detalleTotales ='<tr>
        <td colspan="5" class="textright">Sub_Total </td>
